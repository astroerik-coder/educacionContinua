...
export default function MasterForm({ master, onClose }: MasterFormProps) {
  const { addMaster, updateMaster, loading } = useAcademic()
  ...
  const onSubmit = async (data: MasterFormData) => {
    try {
      if (isEditing) {
        await updateMaster(master.id, data)
      } else {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-medium text-gray-900">{isEditing ? "Editar Maestría" : "Nueva Maestría"}</h3>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Ingrese el título de la maestría"
          />
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-purple-500 focus:border-purple-500"
            placeholder="Describe la maestría"
          />
