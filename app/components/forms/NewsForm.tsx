  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-medium text-gray-900">{isEditing ? "Editar Noticia" : "Nueva Noticia"}</h3>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <input
            type="date"
            {...register("publishDate", { required: "La fecha de publicación es obligatoria" })}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
          />
        ...
        <div>
          ...
          <input
            ...
            {...register("endDate", {
              validate: (value) => {
                ...
                if (publishDate && value) {
                  return new Date(value) > new Date(publishDate) || "La fecha de finalización debe ser posterior a la fecha de publicación";
                }
                return true;
              }
            })}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
          />
          <p className="mt-1 text-xs text-gray-500">Fecha hasta la cual se mostrará la noticia (dejar vacío para mostrar indefinidamente)</p>
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs text-gray-500">Puedes subir una imagen o ingresar una URL abajo</p>
        </div>
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs text-gray-500">URL de una imagen para acompañar la noticia (se usará solo si no subes archivo)</p>
        </div>

        {preview && (
          <div className="mt-2">
            <span className="block text-xs text-gray-500 mb-1">Previsualización:</span>
            <img src={preview} alt="Previsualización" className="max-h-40 rounded shadow" />
          </div>
        )}
